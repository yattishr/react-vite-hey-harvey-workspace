...
export default function Tasks() {
  ...
  const [expandedCompletedTaskIds, setExpandedCompletedTaskIds] = useState<
    Set<number>
  >(new Set());
  ...
  const toggleCompletedTask = (taskId: number) => {
    setExpandedCompletedTaskIds(current => {
            ...
            <Link key={task.id} href={`/tasks/${task.id}`}>
              <Card className="p-6 card-elevated hover:shadow-elevated cursor-pointer transition-all">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {task.status === "completed" && task.result && (
                      <Button
                        ...
                        variant="ghost"
                        className="gap-2"
                        aria-expanded={expandedCompletedTaskIds.has(task.id)}
                        onClick={e => {
                          e.preventDefault();
                          e.stopPropagation();
                      ...
                      >
                        {expandedCompletedTaskIds.has(task.id) ? (
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>
                    {task.assignedAgentCount} agent
                    {task.assignedAgentCount !== 1 ? "s" : ""} assigned
                  </span>
